...
Description: Compute WooCommerce product prices dynamically based on metal rates, weight, wastage, making charge and markup. Supports karat percentages and custom metal formulas (safe evaluator).
Version: 2.0.1
Author: aolo
...
/* ---------------------------
   Admin page (secure: nonces, capability checks, wp_unslash)
--------------------------- */
function dmmp_render_admin_page() {
	...
	if ( isset( $_POST['dmmp_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['dmmp_nonce'] ) ), 'dmmp_save_settings' ) ) {
		...
		// Delete custom metal (if requested)
		...
		echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__( 'Settings saved.', 'dynamic-metal-price-calculator' ) . '</p></div>';
	}

	// Refresh options variable
	$opts = get_option( 'dmmp_options', [] );
	...
	?>
	<div class="wrap">
		<h1><?php echo esc_html( 'Dynamic Metal Price Calculator', 'dynamic-metal-price-calculator' ); ?></h1>
		...
		<form method="post" novalidate>
			...
			<table class="form-table" role="presentation">
				<tr>
					...
					<td><input id="dmmp_gold_rate" name="dmmp_gold_rate" type="number" step="0.01" value="<?php echo esc_attr( $gold_rate ); ?>" class="regular-text" /></td>
				</tr>
				<tr>
					<th><label for="dmmp_silver_rate"><?php echo esc_html__( 'Silver Rate (per gram)', 'dynamic-metal-price-calculator' ); ?></label></th>
					<td><input id="dmmp_silver_rate" name="dmmp_silver_rate" type="number" step="0.01" value="<?php echo esc_attr( $silver_rate ); ?>" class="regular-text" /></td>
				</tr>
				<tr>
					<th><label for="dmmp_gold_22k_percent"><?php echo esc_html__( '22K Percentage (%)', 'dynamic-metal-price-calculator' ); ?></label></th>
					<td><input id="dmmp_gold_22k_percent" name="dmmp_gold_22k_percent" type="number" step="0.01" value="<?php echo esc_attr( $r22 ); ?>" class="small-text" /> <span class="description"><?php echo esc_html__( 'Percent of 24K gold (e.g. 93)', 'dynamic-metal-price-calculator' ); ?></span></td>
				</tr>
				<tr>
					<th><label for="dmmp_gold_20k_percent"><?php echo esc_html__( '20K Percentage (%)', 'dynamic-metal-price-calculator' ); ?></label></th>
					<td><input id="dmmp_gold_20k_percent" name="dmmp_gold_20k_percent" type="number" step="0.01" value="<?php echo esc_attr( $r20 ); ?>" class="small-text" /> <span class="description"><?php echo esc_html__( 'Percent of 24K gold (e.g. 85.5)', 'dynamic-metal-price-calculator' ); ?></span></td>
				</tr>
				<tr>
					<th><label for="dmmp_gold_18k_percent"><?php echo esc_html__( '18K Percentage (%)', 'dynamic-metal-price-calculator' ); ?></label></th>
					<td><input id="dmmp_gold_18k_percent" name="dmmp_gold_18k_percent" type="number" step="0.01" value="<?php echo esc_attr( $r18 ); ?>" class="small-text" /> <span class="description"><?php echo esc_html__( 'Percent of 24K gold (e.g. 78)', 'dynamic-metal-price-calculator' ); ?></span></td>
				</tr>
			</table>

			<?php submit_button( esc_html__( 'Save Rates', 'dynamic-metal-price-calculator' ), 'primary', 'save_rates' ); ?>
		</form>

		<hr/>

		<h2><?php echo esc_html__( 'Add Custom Metal', 'dynamic-metal-price-calculator' ); ?></h2>
		<form method="post" novalidate>
			<?php wp_nonce_field( 'dmmp_save_settings', 'dmmp_nonce' ); ?>
			<table class="form-table" role="presentation">
				<tr>
...
/* ---------------------------
   Product meta fields (select uses current custom metals)
--------------------------- */
add_action( 'woocommerce_product_options_general_product_data', function() {
	...
	woocommerce_wp_text_input( [
		...
		'type'  => 'number',
		'desc_tip' => true,
		'description' => __( 'Weight in grams', 'dynamic-metal-price-calculator' ),
	] );
	...
	woocommerce_wp_text_input( [
		...
		'type'  => 'number',
		'desc_tip' => true,
		'description' => __( 'Optional manual base price to add to metal cost', 'dynamic-metal-price-calculator' ),
	] );
} );

/* Save product meta securely */
add_action( 'woocommerce_admin_process_product_object', function( $product ) {
	...
	$fields = [ '_dmmp_metal_type', '_dmmp_weight', '_dmmp_base_price' ];
	foreach ( $fields as $f ) {
		if ( isset( $_POST[ $f ] ) ) {
			$product->update_meta_data( $f, sanitize_text_field( wp_unslash( $_POST[ $f ] ) ) );
		}
	}
} );

/* ---------------------------
   Safe expression evaluator (shunting-yard + RPN evaluator)
   - No use of eval()
   - Supports + - * / and parentheses and floating numbers
   - Variables must be replaced before calling evaluator
--------------------------- */
function dmmp_tokenize_expression( $expr ) {
	...
	for ( $i = 0; $i < $length; $i++ ) {
		...
		// anything unknown (letters) treat as invalid -> return empty
		return [];
	}
	if ( $number !== '' ) {
		$tokens[] = $number;
...
/**
 * Evaluate a sanitized numeric expression (no variables) safely.
 * Returns float|false on failure.
 */
function dmmp_evaluate_expression_safe( $expr ) {
	// remove whitespace
	$expr = trim( (string) $expr );
	// allow only digits, dots, operators and parentheses
...
/* ---------------------------
   Determine metal rate (uses safe evaluator for custom formulas)
--------------------------- */
	...
	switch ( $type ) {
		...
		default:
			...
			if ( ! empty( $opts['custom_metals'] ) && isset( $opts['custom_metals'][ $type ] ) ) {
				$formula = $opts['custom_metals'][ $type ]['formula'];
				// Replace known variable names with numeric values in a safe pattern:
				// allowed variable names: gold_rate, silver_rate
				$formula_clean = strtolower( (string) $formula );

				// Replace variable tokens with numbers
				$formula_clean = str_ireplace( 'goldrate', (string) $gold_rate, $formula_clean );
				$formula_clean = str_ireplace( 'gold_rate', (string) $gold_rate, $formula_clean );
				$formula_clean = str_ireplace( 'silverrate', (string) $silver_rate, $formula_clean );
				$formula_clean = str_ireplace( 'silver_rate', (string) $silver_rate, $formula_clean );

				// Now evaluate safely
				$val = dmmp_evaluate_expression_safe( $formula_clean );
				if ( $val === false ) {
					return 0;
				}
				return $val;
...
/* ---------------------------
   Price computation (main formula)
   total = (metal_rate * weight) + base_price
   then apply wastage (%), making charge (per g) and markup (%)
--------------------------- */
function dmmp_compute_price_for_product_object( $product ) {
	...
	$making_charge = floatval( get_post_meta( $product_id, '_dmmp_making_charge', true ) );
	...
	// apply wastage (%)
	$total = $total + ( $total * ( $wastage / 100 ) );

	// make charge per gram
	$total = $total + ( $making_charge * $weight );
...
/* ---------------------------
   Persist computed price on product save (so admin shows it)
   and safe checks
--------------------------- */
add_action( 'save_post_product', function( $post_id, $post, $update ) {
	...
	if ( $calc !== null && $calc > 0 ) {
		...
		// update product object safely
		$product->set_price( $calc );
		$product->set_regular_price( $calc );
		// don't call save() here to avoid recursion; WP will persist meta changes
	}
}, 20, 3 );
